...
angular.module('boursesApp').directive('dateAutocomplete', function() {
    return {
        restrict: 'A',
        require: 'ngModel',
        link: function(scope, element, attrs, ngModel) {
            ...
            ngModel.$parsers.unshift(function(viewValue) {
                ...
                if (!_.contains([2, 5], viewValue.length)) {
                    return viewValue;
                }
                if (viewValue.length !== previousValue.length + 1) {
                    return viewValue;
                }
                if (previousValue.lastIndexOf('/') === previousValue.length - 1) {
                    return viewValue;
                }
                if (viewValue.lastIndexOf('/') === viewValue.length - 1) {
                    return viewValue;
                }
